"""
Reference-setting/vectorized GPU baseline metaheuristics for the MVDO project.

Drop-in replacement for algorithms/baselines.py.

Purpose
-------
This file fixes the baseline implementations and sets their internal control
parameters using literature-based settings wherever possible. The common
experimental settings (population size, iterations/function-evaluation budget,
and independent runs) are controlled by the experiment script.

Reference-based defaults used here
----------------------------------
* WOA: whales=30, a decreases from 2 to 0, a2 decreases from -1 to -2.
* SCA: agents=30, number of elites=2.
* HHO: hawks=30, E0 sampled in [-1, 1].
* EO: particles=30, GP=0.5, alpha1=2, alpha2=1.
* AOA: objects=30, C1=2, C2=6, C3=2, C4=0.5.

These are aligned with the parameter table reported in the Archimedes
Optimization Algorithm (AOA) benchmark paper. PSO is kept as the stronger
project baseline requested by the user: linearly decreasing inertia, c1=c2=2,
and velocity clamping for numerical stability on CEC ranges.

Implementation notes
--------------------
* GWO keeps historical Alpha/Beta/Delta leaders, consistent with preserving the
  best solutions found so far.
* WOA uses scalar A, C, l, and p per whale/run. l is sampled from [a2, 1], where
  a2 linearly decreases from -1 to -2, as in common original WOA code.
...
class PSO(BaseOptimizer):
    """Particle Swarm Optimization, project-selected strong baseline.

    Defaults:
        w decreases linearly from 0.9 to 0.4
        c1 = 2, c2 = 2
        velocity clamp = 20% of the variable range

    We keep this stronger PSO as requested, rather than weakening it to match
    older PSO tables. This helps avoid an unfairly easy comparison for MVDO.
    """
...
class GWO(BaseOptimizer):
    ...
    Reference setting: convergence constant a decreases linearly from 2 to 0.
    The implementation keeps historical Alpha/Beta/Delta leaders to preserve the
    best solutions found so far.
    """
...
class WOA(BaseOptimizer):
    """Whale Optimization Algorithm with reference-style parameters.

    Literature/default parameters:
        whales = 30  (provided by experiment script)
        a      = linearly decreases from 2 to 0
        a2     = linearly decreases from -1 to -2
        b      = 1

    Scalar A, C, l, and p are sampled per whale/run and broadcast over
    dimensions. This follows the common MATLAB-style implementation more closely
    than drawing a separate coefficient for each variable.
    """
    ...
    def step(self, t):
        ...
        denom = max(1, self.max_iter - 1)
        a = 2.0 - 2.0 * (t / denom)
        a2 = -1.0 - 1.0 * (t / denom)  # from -1 to -2
        ...
        C = 2.0 * r2
        p = self._rand(R, N, 1)
        l = (a2 - 1.0) * self._rand(R, N, 1) + 1.0  # l in [a2, 1]
...
class SCA(BaseOptimizer):
    """Sine Cosine Algorithm with reference-style elite count.

    Literature/default parameters from the AOA benchmark table:
        agents = 30  (provided by experiment script)
        number of elites = 2

    Standard SCA normally moves agents toward the best destination point. To
    implement the reported number of elites while staying close to SCA behavior,
    each agent samples its destination from a historical top-`num_elites` pool.
    With num_elites=1 this reduces to the standard best-destination version.
    """

    def __init__(self, fn, dim, lb, ub, pop_size=30, max_iter=1000, num_runs=30,
                 a=2.0, num_elites=2, device='cuda:0', dtype=torch.float64, seed=None):
        super().__init__(fn, dim, lb, ub, pop_size, max_iter, num_runs, device, dtype, seed)
        self.a = a
    ...
    def _update_elites(self):
        k = max(1, self.num_elites)
        top, top_fit = _topk_positions(self.population, self.fitness, k)
        if self.elite_pos is None:
            self.elite_pos = top
            self.elite_fit = top_fit
        else:
            self.elite_pos, self.elite_fit = _topk_pool(
                self.elite_pos, self.elite_fit, self.population, self.fitness, k
            )
    ...
    def step(self, t):
        ...
        elite_idx = self._randint(0, max(1, self.num_elites), R, N)
        run_idx = torch.arange(R, device=self.device).view(R, 1).expand(R, N)
        target = self.elite_pos[run_idx, elite_idx]
