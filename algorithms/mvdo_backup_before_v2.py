...
class MVDO(BaseOptimizer):
    ...
    def _phase2_spiral(self, X, V, r, rho, phi):
        ...
        v_theta = (self.C / (rho + self.rho_core)).unsqueeze(-1)

        # Radial shrink. phi/2 moves from 1 to 0, so radial pull is strongest early
        # and weakens late, supporting local refinement near promising vortices.
    ...
    def _splash_out(self, X, V_best):
        """Re-launch stuck agents via Levy flight around the alpha vortex."""
        R, N, D = X.shape
        levy = _mantegna_levy((R, N, D), self.levy_beta, self.gen, self.device, self.dtype)
        return V_best.unsqueeze(1) + (self.L_splash / math.sqrt(max(D, 1))) * levy
    ...
    def step(self, t):
        ...
        # Optional greedy agent update. Disabled by default to preserve the original
        # exploratory population dynamics.
